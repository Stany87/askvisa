...
if (isset($_POST['create_order'])) {
    try {
        ...
        // Create order in database - REMOVED total_people from INSERT
        ...
        $stmt->execute([$country_id, $order_contact_email, $order_contact_phone, $payment_amount, $currency]);
    ...
    try {
        ...
        if ($order_id && $razorpay_payment_id && $razorpay_order_id && $razorpay_signature) {
            ...
            if ($generated_signature === $razorpay_signature) {
                // Payment verified successfully - update in a single transaction
                $pdo->beginTransaction();
                
                try {
                    // First, update visa_order with payment status
                    $stmt = $pdo->prepare("UPDATE visa_orders SET updated_at = NOW(), payment_status = 'paid' WHERE id = ?");
                    $stmt->execute([$order_id]);
                    
                    // Then insert payment record
                    $stmt = $pdo->prepare("INSERT INTO payments 
                        (order_id, provider, provider_payment_id, amount, currency, status) 
                        VALUES (?, 'razorpay', ?, 
                        (SELECT total_amount FROM visa_orders WHERE id = ?), 
                        (SELECT currency FROM visa_orders WHERE id = ?), 'success')");
                    $stmt->execute([$order_id, $razorpay_payment_id, $order_id, $order_id]);
                ...
                $response = ['success' => false, 'message' => 'Payment verification failed: Invalid signature'];
            }
        } else {
            $response = ['success' => false, 'message' => 'Missing payment parameters'];
        }
    } catch (Exception $e) {
...
// Handle complete after payment
if (isset($_POST['complete_after_payment'])) {
    $order_id = $_SESSION['current_order_id'] ?? 0;
    
    if ($order_id) {
        try {
            // First, save all applicant data to database
            $pdo->beginTransaction();
            
            // Get order details (don't need total_people since it's not in DB)
            $total_people = $_SESSION['total_people'] ?? 1;
            
            // Save all applicants data
            for ($i = 1; $i <= $total_people; $i++) {
                if (isset($_SESSION['collected_info']["applicant_$i"])) {
                    $applicant = $_SESSION['collected_info']["applicant_$i"];
                    
                    // Insert applicant
                    $stmt = $pdo->prepare("INSERT INTO applicants 
                        (order_id, applicant_no, applicant_email, applicant_phone, visa_status) 
                        VALUES (?, ?, ?, ?, 'submitted')");
                    $stmt->execute([$order_id, $i, $applicant['email'] ?? '', $applicant['phone'] ?? '']);
                    
                    $applicant_id = $pdo->lastInsertId();
                    
                    // Save answers and files
                    if (isset($applicant['answers'])) {
                        foreach ($applicant['answers'] as $question_id => $answer) {
                            // Get question type from session
                            $question_type = $_SESSION['question_data'][$question_id]['field_type'] ?? 'text';
                            
                            if (strpos($answer, 'fetch_file.php') !== false) {
                                // It's a file - store in applicant_files
                                $stmt = $pdo->prepare("INSERT INTO applicant_files 
                                    (order_id, applicant_id, question_id, file_path) 
                                    VALUES (?, ?, ?, ?)");
                                $stmt->execute([$order_id, $applicant_id, $question_id, $answer]);
                                
                                // Also store reference in applicant_answers with file type
                                ...
                                $stmt = $pdo->prepare("INSERT INTO applicant_answers 
                                    (order_id, applicant_id, question_id, answer_type, answer_text) 
                                    VALUES (?, ?, ?, ?, ?)");
                                $stmt->execute([$order_id, $applicant_id, $question_id, $question_type, $answer]);
                            }
                        }
                    }
                }
            ...
            if (isset($_SESSION['order_folder_name'])) {
                ...
                if (is_dir($base_path . $old_folder_name)) {
                    ...
                    $stmt = $pdo->prepare("UPDATE applicant_files SET file_path = REPLACE(file_path, ?, ?) WHERE order_id = ?");
                    $stmt->execute([$old_folder_name, $new_folder_name, $order_id]);
                    
                    $stmt = $pdo->prepare("UPDATE applicant_answers SET answer_text = REPLACE(answer_text, ?, ?) WHERE order_id = ? AND answer_type = 'file'");
                    $stmt->execute([$old_folder_name, $new_folder_name, $order_id]);
                }
            }
            
            // Update order visa status
            $stmt = $pdo->prepare("UPDATE visa_orders SET visa_status = 'submitted', updated_at = NOW() WHERE id = ?");
            $stmt->execute([$order_id]);
    ...
    if ($order_id) {
        ...
        try {
            ...
            if ($order_info) {
                ...
                // Count applicants for summary display
                $data['total_people'] = count($applicants);
                
                foreach ($applicants as $applicant) {
                    $applicant_key = "applicant_" . $applicant['applicant_no'];
                    
                    // Get applicant's answers from both applicant_answers and applicant_files
                    $stmt = $pdo->prepare("SELECT 
                        q.id as question_id,
                        q.label,
                        q.field_key,
...
// Function to calculate payment amount - ₹100 per applicant
function calculatePaymentAmount($country_name, $num_applicants) {
    $per_applicant = 100; // ₹100 per applicant
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (empty($upload_errors)) {
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
                // Store as /fetch_file.php?path=encoded_path
...
<script>
    ...
    async function initiatePayment(amount, currency) {
        try {
            ...
            if (orderData.success && orderData.order_id) {
                ...
                // Create Razorpay order using your server endpoint
                ...
                if (razorpayData.success && razorpayData.razorpay_order_id) {
                    ...
                    const options = {
                        key: 'rzp_test_SA4dsulyUy16xi', // Your Razorpay Key ID
                        ...
                        handler: async function(response) {
                            ...
                            const verifyResponse = await fetch('?ajax=1', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded',
                                },
                            ...
                            if (verifyData.success) {
                                ...
                                await completeApplicationAfterPayment();
                            } else {
                        ...
                        prefill: {
                            email: '<?php echo $_SESSION["order_contact_email"] ?? ""; ?>',
                            contact: '<?php echo $_SESSION["order_contact_phone"] ?? ""; ?>'
                        },
                        theme: {
                            color: '#4361ee'
                        },
                ...
                throw new Error(orderData.message || 'Failed to create application order');
            }
        } catch (error) {
    ...
    // Complete application after successful payment
    async function completeApplicationAfterPayment() {
        try {
            ...
            if (data.success) {
                ...
                botRow.innerHTML = `
                    ...
                    <div class="message-content">
                        <div class="message-text">Payment successful! ✅<br>Your application has been submitted successfully!<br>Order ID: <b>#${data.order_id}</b></div>
                ...
                throw new Error(data.message || 'Failed to complete application');
            }
        } catch (error) {
            console.error('Error completing application:', error);
            alert('Error completing application: ' + error.message);
        }
    }
                                            ...
                                            <div class="file-preview">
                                                ...
                                                ` : `
                                                    <div style="text-align: center;">
                                                        ...
                                                        <p style="font-size: 10px; color: var(--gray);">${fileName}</p>
                                                    </div>
                                                `}
                                            </div>
                                        </div>
    ...
    async function downloadSummaryAsPDF() {
        try {
            // Create a simplified version of the summary for PDF
            let pdfHtml = `
                <!DOCTYPE html>
                <html>
                <head>
            ...
            if (window.summaryData && window.summaryData.order_info) {
                pdfHtml += `
                    <div class="section">
                        ...
                        <table class="info-table">
                            <tr>
                                ...
                                <td class="info-value">${window.summaryData.order_info.id || 'N/A'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Country</td>
                                <td class="info-value">${window.summaryData.country_name || 'N/A'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Primary Contact Email</td>
                                <td class="info-value">${window.summaryData.order_info.email || 'N/A'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Primary Contact Phone</td>
                                <td class="info-value">${window.summaryData.order_info.phone || 'N/A'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Payment Status</td>
                                <td class="info-value ${window.summaryData.order_info.payment_status === 'paid' ? 'status-paid' : 'status-pending'}">
                                    ${window.summaryData.order_info.payment_status === 'paid' ? 'PAID' : 'PENDING'}
                                </td>
                            </tr>
                `;
                
                if (window.summaryData.order_info.total_amount) {
                    pdfHtml += `
                            <tr>
                                <td class="info-label">Amount Paid</td>
                                <td class="info-value">${window.summaryData.order_info.currency} ${window.summaryData.order_info.total_amount}</td>
                            </tr>
                    `;
                }
                ...
                pdfHtml += `
                            <tr>
                                ...
                                <td class="info-value">${window.summaryData.order_info.visa_status || 'initiated'}</td>
                            </tr>
                        </table>
                    </div>
                `;
            ...
            if (window.summaryData && window.summaryData.payment_info) {
                pdfHtml += `
                    <div class="section">
                        ...
                        <table class="info-table">
                            <tr>
                                ...
                                <td class="info-value">${window.summaryData.payment_info.provider_payment_id || 'N/A'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Payment Provider</td>
                                <td class="info-value">${window.summaryData.payment_info.provider || 'Razorpay'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Amount Paid</td>
                                <td class="info-value">${window.summaryData.payment_info.currency} ${window.summaryData.payment_info.amount}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Payment Status</td>
                                <td class="info-value">${window.summaryData.payment_info.status ? window.summaryData.payment_info.status.toUpperCase() : 'N/A'}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Payment Date</td>
                                <td class="info-value">${window.summaryData.payment_info.created_at ? new Date(window.summaryData.payment_info.created_at).toLocaleString() : 'N/A'}</td>
                            </tr>
                        </table>
                    </div>
                `;
            ...
            if (window.summaryData && window.summaryData.data) {
                const applicantKeys = Object.keys(window.summaryData.data).filter(key => key.startsWith('applicant_'));
                
                applicantKeys.forEach((applicantKey, index) => {
                    const applicant = window.summaryData.data[applicantKey];
                    const applicantNum = applicantKey.replace('applicant_', '');
                    ...
                    pdfHtml += `
                        <div class="section">
                            <h2>Applicant #${applicantNum}</h2>
                            <table class="info-table">
                                <tr>
                                    <td class="info-label">Email</td>
                                    <td class="info-value">${applicant.email || 'N/A'}</td>
                                </tr>
                                <tr>
                                    <td class="info-label">Phone</td>
                                    <td class="info-value">${applicant.phone || 'N/A'}</td>
                                </tr>
                                <tr>
                                    <td class="info-label">Visa Status</td>
                                    <td class="info-value">${applicant.visa_status || 'submitted'}</td>
                                </tr>
                            </table>
                    `;
                    ...
                    // Add answers
                    if (applicant.answers) {
                        pdfHtml += `<h3>Application Details</h3>
                            <div class="applicant-section">`;
                        
                        Object.keys(applicant.answers).forEach(qId => {
                            const answer = applicant.answers[qId];
                            const label = window.summaryData.question_labels ? (window.summaryData.question_labels[qId] || `Question ${qId}`) : `Question ${qId}`;
                            ...
                            if (answer.includes('fetch_file.php')) {
                                ...
                                pdfHtml += `
                                    <div class="answer-item">
                                        ...
                                        <div class="file-info">
                                            ${fileName} • ${isPdf ? 'PDF Document' : 'Image File'}
                                        </div>
                                    </div>
                                `;
                            } else {
            ...
            pdfHtml += `
                <div class="footer">
                    <p>This document was automatically generated by Ask Visa Portal</p>
                    <p>Document ID: ${currentOrderId || 'N/A'} | Generated: ${new Date().toLocaleString()}</p>
                </div>
                    </div>
                </body>
                </html>
            ...
            setTimeout(() => {
                const opt = {
                    margin: [10, 10, 10, 10],
                    filename: `visa_application_summary_${currentOrderId || 'summary'}.pdf`,
                ...
                // Generate and save PDF from the iframe
            ...
            alert('Unable to generate PDF. Please try again or take a screenshot of the summary.');
        }
    }
