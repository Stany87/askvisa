...
function processPayment($order_id, $amount = 0) {
...
if (isset($_GET['ajax'])) {
        ...
        switch ($_SESSION['step']) {
            ...
            case 'order_phone':
                ...
                if ($validation_error) {
                    ...
                    $_SESSION['order_currency'] = 'USD'; // Default currency
            ...
            case 'payment_now':
                ...
                if ($payment_result['success']) {
                    ...
                    try {
                        ...
                        // 1. Create the Order Entry with payment info
                        ...
                        $stmt->execute([
                            $order_id,
                            $_SESSION['provider_payment_id'],
                            $_SESSION['order_amount'],
                            $_SESSION['order_currency']
                        ]);
                
                        // 3. Rename the Physical Folder
                        $home_dir = dirname($_SERVER['DOCUMENT_ROOT']); 
                            ...
                            foreach ($app_data['answers'] as $q_id => $val) {
                                if (strpos($val, 'fetch_file.php') !== false) {
                                    ...
                                    if ($old_path) {
                                        ...
                                        $new_path = str_replace($old_folder_name, $new_folder_name, $old_path);
                                        ...
                                        $field_type = 'file';
                                        if (isset($_SESSION['question_data'][$q_id])) {
                                            $field_type = $_SESSION['question_data'][$q_id]['field_type'];
                                        }
                        ...
                        $pdo->commit();
                        $response = "Payment successful! ✅ Transaction ID: **" . $payment_result['transaction_id'] . "**\n";
                        $response .= "Order ID: **$order_id**\n";
                        $response .= "Your application has been submitted successfully!";
                        $_SESSION['step'] = 'finish';
                    } catch (Exception $e) { 
                        $pdo->rollBack(); 
                        $response = "Payment successful but error saving order: " . $e->getMessage();
    ...
    switch ($_SESSION['step']) {
        ...
        case 'payment_now': $step_count .= "8/8"; break;
...
<script>
    ...
    async function processPaymentNow() {
        const payNowBtn = document.querySelector('.paynow-button');
        if (!payNowBtn || payNowBtn.disabled) return;
        ...
        try {
            // Send an empty message to trigger payment processing
            const formData = new FormData();
            formData.append('message', 'pay_now');
            ...
            errorRow.innerHTML = `
                ...
                <div class="message-content">
                    <div class="message-text">Sorry, an error occurred during payment. Please try again.</div>
                    <div class="message-time">${new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                </div>
            `;
    ...
    async function sendMessage() {
        ...
        try {
            ...
            // Check if we need to show Pay Now button
