    ...
    try {
        ...
        if ($order_id && $razorpay_payment_id && $razorpay_order_id && $razorpay_signature) {
            ...
            if ($generated_signature === $razorpay_signature) {
                ...
                // First, update the visa_order payment status (trigger will handle this)
                ...
                $stmt = $pdo->prepare("UPDATE visa_orders SET updated_at = NOW() WHERE id = ?");
...
if (isset($_POST['complete_after_payment'])) {
    ...
    if ($order_id) {
        // Update order visa status to initiated (payment already updated via trigger)
        $stmt = $pdo->prepare("UPDATE visa_orders SET visa_status = 'initiated', updated_at = NOW() WHERE id = ?");
        $stmt->execute([$order_id]);
...
if (isset($_GET['ajax'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (!isset($_SESSION['order_folder_name'])) {
            $_SESSION['order_folder_name'] = 'order_' . time();
        }
        ...
        if (empty($upload_errors)) {
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
                // Store relative path as per your example: /home/u261509590/domains/askvisa.in/gov_id/2026/01/21/order_22/applicant_1/file_1768987050_697099aa79614.jpeg
                $relative_file_path = $sub_path . '/' . $filename;
                $img_path = $relative_file_path;
            }
        } else {
            $response = implode("\n", $upload_errors);
        }
    ...
    if ($msg !== '' || $img_path !== '' || $is_select_selection) {
        ...
        if ($img_path !== '') {
            ...
            $_SESSION['messages'][] = [
                ...
                'img' => '/fetch_file.php?path=' . urlencode($img_path), 
                'is_pdf' => $is_pdf
            ];
        } else if ($msg !== '') {
            if ($is_select_selection) {
        ...
        switch ($_SESSION['step']) {
            case 'country':
                ...
                if ($country) {
                    ...
                    $q_stmt = $pdo->prepare("SELECT id, label, field_type, field_key, is_required FROM country_questions WHERE country_id = ? ORDER BY sort_order ASC");
            ...
            case 'details':
                ...
                $is_required = $current_q['is_required'] ?? 1;
            ...
            case 'order_email':
                if (!filter_var($msg, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $response = "What is the **Primary Contact Phone Number** for the order?";
                }
                break;
    ...
    switch ($_SESSION['step']) {
        ...
        case 'details':
            if (isset($_SESSION['db_questions'][$_SESSION['q_idx']])) {
                ...
                // Check if current question is a date question (based on field_type)
                if ($current_q['field_type'] === 'date') {
                    ...
                    const options = {
                        ...
                        handler: async function(response) {
                            ...
                            if (verifyData.success) {
                                ...
                                completeApplicationAfterPayment();
                            } else {
    ...
    async function showSummary() {
        try {
            ...
            if (data.success) {
                ...
                // Applicants Information - FIXED: Only show actual applicants
                if (data.data) {
                    // Get the actual number of applicants from the data
                ...
                <div style="margin-bottom: 30px; padding-bottom: 20px; border-bottom: 1px solid #eee; page-break-after: avoid;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px;">
                        ...
                        <div style="margin-bottom: 10px;">
                            ...
                            <div style="background: #f8f9fa; padding: 10px 12px; border-radius: 6px; border-left: 3px solid ${data.order_info.payment_status === 'paid' ? '#28a745' : '#dc3545'}; font-weight: 500; color: ${data.order_info.payment_status === 'paid' ? '#28a745' : '#dc3545'}">
                                ${data.order_info.payment_status === 'paid' ? '✅ Paid' : '❌ Pending'}
                            </div>
                        </div>
                        ${data.order_info.total_amount ? `
                        <div style="margin-bottom: 10px;">
                            ...
                            <div style="background: #f8f9fa; padding: 10px 12px; border-radius: 6px; border-left: 3px solid #4361ee; font-weight: 500;">
                                ${data.order_info.currency} ${data.order_info.total_amount}
                            </div>
                        </div>
                        ` : ''}
                        <div style="margin-bottom: 10px;">
                            ...
                            <div style="background: #f8f9fa; padding: 10px 12px; border-radius: 6px; border-left: 3px solid #4361ee; font-weight: 500; color: #4361ee">
                                ${data.order_info.visa_status || 'initiated'}
                            </div>
                        </div>
                    </div>
