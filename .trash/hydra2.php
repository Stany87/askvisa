...
if (isset($_POST['create_order'])) {
    try {
        ...
        // Create order in database WITHOUT total_people (not in your schema)
        ...
        // Also save all applicant data immediately when creating order
        if (isset($_SESSION['collected_info']) && !empty($_SESSION['collected_info'])) {
            foreach ($_SESSION['collected_info'] as $applicant_key => $applicant_data) {
                if (preg_match('/applicant_(\d+)/', $applicant_key, $matches)) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO applicants 
                        ...
                        VALUES (?, ?, ?, ?, 'submitted')");
                    $stmt->execute([$order_id, $applicant_no, $applicant_email, $applicant_phone]);
                    
                    $applicant_id = $pdo->lastInsertId();
                    
                    // Save answers and files to database
                    if (isset($applicant_data['answers'])) {
                        foreach ($applicant_data['answers'] as $question_id => $answer) {
                            // Check if it's a file path
                            if (strpos($answer, 'fetch_file.php') !== false || strpos($answer, '.jpg') !== false || strpos($answer, '.jpeg') !== false || strpos($answer, '.png') !== false || strpos($answer, '.pdf') !== false) {
    ...
    try {
        ...
        if ($order_id && $razorpay_payment_id && $razorpay_order_id && $razorpay_signature) {
            ...
            if ($generated_signature === $razorpay_signature) {
                ...
                // Update the visa_order payment status (trigger will handle this)
                ...
                // Update visa_order with payment ID
...
if (isset($_POST['complete_after_payment'])) {
    ...
    if ($order_id) {
        try {
            ...
            // Update order visa status to completed
            $stmt = $pdo->prepare("UPDATE visa_orders SET visa_status = 'completed', updated_at = NOW() WHERE id = ?");
            $stmt->execute([$order_id]);
            
            // Update all applicants' visa status to completed
            $stmt = $pdo->prepare("UPDATE applicants SET visa_status = 'completed' WHERE order_id = ?");
            $stmt->execute([$order_id]);
...
<div id="privacyPopup" class="privacy-popup">
    <div class="privacy-container">
        <div class="privacy-header">
            <h3><i class="fas fa-shield-alt"></i> Privacy Policy - Ask Visa (FlyAnyTrip)</h3>
            <div class="privacy-close" onclick="closePrivacyPolicy()">
                <i class="fas fa-times"></i>
            </div>
        </div>
        <div class="privacy-content">
            <h4>1. Introduction</h4>
            <p>Welcome to Ask Visa, operated by FlyAnyTrip ("we," "our," or "us"). We are committed to protecting your personal information and your right to privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our visa application service.</p>
            
            <h4>2. Information We Collect</h4>
            <h5>Personal Information</h5>
            <ul>
                <li><strong>Contact Information:</strong> Name, email address, phone number, postal address</li>
                <li><strong>Identification Documents:</strong> Passport details, national ID, driver's license</li>
                <li><strong>Travel Information:</strong> Travel dates, destination countries, purpose of travel</li>
                <li><strong>Financial Information:</strong> Payment details for processing visa fees</li>
            </ul>
            
            <h5>Sensitive Information</h5>
            <ul>
                <li>Biometric data (photos, fingerprints when required)</li>
                <li>Government-issued identification documents</li>
                <li>Travel history and previous visa information</li>
            </ul>
            
            <h4>3. How We Use Your Information</h4>
            <p>We use the information we collect for the following purposes:</p>
            <ul>
                <li>To process and submit your visa applications to appropriate government authorities</li>
            ...
            <ul>
                <li><strong>Encryption:</strong> All sensitive data is encrypted using industry-standard encryption protocols (AES-256)</li>
                <li><strong>Secure Storage:</strong> Documents are stored in secure, access-controlled servers</li>
                ...
                <li><strong>Data Minimization:</strong> We only collect information necessary for visa processing</li>
            </ul>
            
            <h4>5. Data Sharing and Disclosure</h4>
            <p>We may share your information with:</p>
            <ul>
                <li><strong>Government Authorities:</strong> Required for visa processing and compliance</li>
            ...
            <ul>
                <li><strong>Active Applications:</strong> Retained for 90 days after visa decision</li>
                <li><strong>Completed Applications:</strong> Archived for 7 years for legal compliance</li>
            ...
            <p>Our services are not directed to individuals under 18. We do not knowingly collect personal information from children. If you become aware that a child has provided us with personal information, please contact us immediately.</p>
            
            <h4>11. Policy Updates</h4>
            <p>We may update this Privacy Policy periodically. We will notify you of significant changes through email or prominent notices on our service.</p>
            
            <h4>12. Contact Information</h4>
            <p>For privacy-related inquiries, please contact our Data Protection Officer:</p>
            <ul>
                <li><strong>Company:</strong> FlyAnyTrip</li>
                <li><strong>Founder:</strong> Anshuman Singh</li>
                <li><strong>Email:</strong> privacy@askvisa.in</li>
                <li><strong>Website:</strong> https://www.flyanytrip.com/</li>
                <li><strong>Phone:</strong> +91-7880789486</li>
                <li><strong>Address:</strong> Orchid Plaza, 321, B/H MacDonald, Sama-Savli Road, Vadodara, Gujarat 390024, India</li>
            </ul>
        </div>
